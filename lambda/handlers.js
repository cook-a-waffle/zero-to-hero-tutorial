    ...
    async handle(handlerInput) {
        ...
        if (dateAvailable){
                    ...
                    datasources: {
                        launchData: {
                            ...
                            properties: {
                                ...
                                hintString: handlerInput.t('LAUNCH_HINT_MSG'),
                                logoImage: Viewport.pixelWidth > 480 ? util.getS3PreSignedUrl('ZeroToHero/full_icon_512.png') : util.getS3PreSignedUrl('ZeroToHero/full_icon_108.png'),
                                backgroundImage: util.getS3PreSignedUrl('ZeroToHero/straws_'+resolution+'.png'),
            ...
            handlerInput.responseBuilder.withStandardCard(
                ...
                util.getS3PreSignedUrl('ZeroToHero/straws_480x480.png'));
        } else {
            speechText += handlerInput.t('MISSING_MSG');
            // we use intent chaining to trigger the birthday registration multi-turn
            handlerInput.responseBuilder.addDelegateDirective({
...
const CelebrityBirthdaysIntentHandler = {
    ...
    async handle(handlerInput) {
        ...
        if (util.supportsAPL(handlerInput) && speechResponse) { // empty speechResponse -> no API results
            speechText += handlerInput.t('POST_CELEBRITIES_APL_HELP_MSG');
            const {Viewport} = handlerInput.requestEnvelope.context;
            const resolution = Viewport.pixelWidth + 'x' + Viewport.pixelHeight;
            handlerInput.responseBuilder.addDirective({
                type: 'Alexa.Presentation.APL.RenderDocument',
                ...
                datasources: {
                    listData: {
                        ...
                        properties: {
                            config: {
                                backgroundImage: util.getS3PreSignedUrl('ZeroToHero/lights_'+resolution+'.png'),
                                title: handlerInput.t('LIST_HEADER_MSG'),
                                skillIcon: util.getS3PreSignedUrl('ZeroToHero/full_icon_108.png'),
                                hintText: handlerInput.t('LIST_HINT_MSG')
                            },
                            list: {
                                listItems: response.results.bindings
                            }
                        },
                        transformers: [{
            ...
            handlerInput.responseBuilder.withStandardCard(
                ...
                speechResponse,
                util.getS3PreSignedUrl('ZeroToHero/lights_480x480.png'));
        } else {
